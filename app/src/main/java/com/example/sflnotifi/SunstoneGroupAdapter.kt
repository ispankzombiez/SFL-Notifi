...
class SunstoneGroupAdapter : RecyclerView.Adapter<SunstoneGroupAdapter.ViewHolder>() {
    private var sunstoneGroups: List<SunstoneGroup> = emptyList()
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val group = sunstoneGroups[position]
        val binding = holder.binding

        // Set sunstone count
        binding.cropNameText.text = "${group.plots.size}x Sunstones"
        ...
        binding.root.setOnClickListener {
            ...
            val newGroups = sunstoneGroups.toMutableList()
            newGroups[position] = group.copy(isExpanded = !group.isExpanded)
            
            // Calculate the diff and update
            val diffCallback = SunstoneGroupDiffCallback(sunstoneGroups, newGroups)
            val diffResult = DiffUtil.calculateDiff(diffCallback)
            
            sunstoneGroups = newGroups
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int, payloads: List<Any>) {
        if (payloads.contains(PAYLOAD_TIME_UPDATE)) {
            updateTimeDisplay(holder.binding, sunstoneGroups[position])
        } else {
            onBindViewHolder(holder, position)
        }
    }

    private fun updateTimeDisplay(binding: ItemCropGroupBinding, group: SunstoneGroup) {
    ...
    override fun getItemCount() = sunstoneGroups.size

    fun updateGroups(newGroups: List<SunstoneGroup>) {
        val diffCallback = SunstoneGroupDiffCallback(sunstoneGroups, newGroups)
        val diffResult = DiffUtil.calculateDiff(diffCallback)
        sunstoneGroups = newGroups
        diffResult.dispatchUpdatesTo(this)
    ...
    fun updateTimeDisplays() {
        notifyItemRangeChanged(0, sunstoneGroups.size, PAYLOAD_TIME_UPDATE)
    }

    private class SunstoneGroupDiffCallback(
        private val oldList: List<SunstoneGroup>,
        private val newList: List<SunstoneGroup>
    ) : DiffUtil.Callback() {
    ...
    fun getGroups(): List<SunstoneGroup> {
        return sunstoneGroups.toList()
    }
} 